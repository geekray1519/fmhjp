...
export function CategoryCard({ category, index }: CategoryCardProps) {
  ...
  return (
    <Link
      href={`/${category.slug}`}
      className="group relative block p-5 sm:p-6 rounded-2xl border border-border bg-card hover:bg-card-hover overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-accent/5 hover:-translate-y-1 animate-fade-in category-card-hover"
    ...
    >
      ...
      <div className={`absolute inset-0 opacity-0 group-hover:opacity-[0.04] transition-opacity duration-300 bg-gradient-to-br ${category.color}`} />
      {/* Decorative corner glow */}
      <div className={`absolute -bottom-8 -right-8 w-24 h-24 opacity-0 group-hover:opacity-[0.08] transition-opacity duration-500 bg-gradient-to-br ${category.color} rounded-full blur-2xl`} />
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-3">
          <span className="text-3xl group-hover:scale-110 transition-transform duration-300">{category.icon}</span>
          <ChevronRight
            size={18}
            className="text-muted group-hover:text-accent group-hover:translate-x-1 transition-all duration-300"
          />
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] font-medium text-muted flex-wrap">
